...
class TheAllowList(Responder):
    def __init__(self):
        """Initialize a The-Allow-List responder.
        """
        ...
        self.category_id_fp = self.get_param("config.category_id_fp", None, "Forcepoint ID de la catégorie concernant la liste des URLs refusées.")
    ...
    # Forcepoint #
    ##############
    def allow_data_fp(self, data_to_allow):
        """
        Adds a URL or a domain name to the Forcepoint allow list within a specific category.

        Args:
           data_to_allow (str): The data to be allowed via Forcepoint.
        ...
        with open(filename, 'r') as f:
            data = f.read()
            requests.post(
                f"{FORCEPOINT_API_URL}/delete/urls",
                auth=(self.fp_username, self.fp_password),
                data=data,
                proxies=self.proxies,
                verify=False,
    ...
    def remove_info_observable(self, observable_id, observable_tags):
        """
        ...
        """
        observable_tags.remove("Denied : Forcepoint")
    ...
    def run(self):
        """Run a The-Allow-List responder.
        """
        Responder.run(self)
        ...
        if self.get_param("data.dataType") == "url" or self.get_param("data.dataType") == "domain":
            ...
            data_to_allow = self.get_param("data.data")

            ##############
            # Forcepoint #
            ##############
            if "Blocked : Forcepoint" in observable_tags:
                response_allow_data_fp = self.allow_data_fp(data_to_allow)

                if response_allow_data_fp == 0:
                    self.remove_info_observable(observable_id, observable_tags)
                    self.report({"message": "Allowed in Forcepoint."})
                else:
                    self.report({"message": "Wasn't allowed in Forcepoint. An error occured. Exit."})
                    return
            else:
                self.report({"message": "Already allowed in Forcepoint."})
        else:
